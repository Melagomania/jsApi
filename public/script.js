...
Geolocation.prototype.startWatch = function () {
    ...
    this.locationWatch = navigator.geolocation.watchPosition(function (position) {
        ...
        _this.locationHistory.add(posStr);
        if (_this.locationHistory.size === 10) {
            _this.stopWatching();
        }
...
Geolocation.prototype.resetMapImg = function () {
    this.mapImg.setAttribute('src', 'waiting.gif');
};
...
Geolocation.prototype.getAPILink = function (data) {
    ...
    data.forEach(function (i) {
        src += '&markers=label:' + labelNumber++ + '%7C' + i;
    });
    return src;
};
